...
inline BOOL CallDelegateWithArgs(NSObject **delegate, SEL selector, NSArray *args)
{
...
inline CGRect CGRectResizeToCGSize(CGRect rect, CGSize size) {
    CGRect newrect = rect;
    newrect.size = size;
    return newrect;
}

inline CGRect CGRectResizeToWidth(CGRect rect, CGFloat width)
{
    CGRect newrect = rect;
...
inline CGRect CGRectResizeToHeight(CGRect rect, CGFloat height)
{
    CGRect newrect = rect;
...
inline CGRect CGRectMoveToCGPoint(CGRect rect, CGPoint point) {
    CGRect newrect = rect;
    newrect.origin = point;
    return newrect;
}

inline CGRect CGRectMoveToXPosition(CGRect rect, CGFloat x)
{
    CGRect newrect = rect;
...
inline CGRect CGRectMoveToYPosition(CGRect rect, CGFloat y)
{
    CGRect newrect = rect;
...
inline CGSize CGSizeScaleTo(CGSize size, CGFloat scale) {
    CGSize newsize = size;
...
inline CGSize CGSizeScaleToScaleFactors(CGSize size, CGFloat widthFactor, CGFloat heightFactor) {
    CGSize newsize = size;
...
inline CGPoint CGRectGetCenter(CGRect rect) {
	...
	p.x += rect.size.width / 2.0;
	p.y += rect.size.height / 2.0;
	return p;
}

inline void PrintCallStackSymbols(int depth) {
    NSArray *ar = [NSThread callStackSymbols];
    NSLog(@"%s %@", __func__, (([ar count] > depth && (depth > 0)) ? [ar subarrayWithRange:NSMakeRange(0, depth)] : ar));
}

#if TARGET_OS_IPHONE
inline CGFloat XOffsetFromUIView(UIView *view)
#else
inline CGFloat XOffsetFromUIView(NSView *view)
#endif
{
...
#if TARGET_OS_IPHONE
inline CGFloat YOffsetFromUIView(UIView *view)
#else
inline CGFloat YOffsetFromUIView(NSView *view)
#endif
{
...
#if TARGET_OS_IPHONE
inline CGFloat HeightFromUIView(UIView *view)
#else
inline CGFloat HeightFromUIView(NSView *view)
#endif
{
...
#if TARGET_OS_IPHONE
inline CGFloat WidthFromUIView(UIView *view)
#else
inline CGFloat WidthFromUIView(NSView *view)
#endif
{
